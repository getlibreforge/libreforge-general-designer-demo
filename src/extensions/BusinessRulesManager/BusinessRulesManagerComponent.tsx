...
const BusinessRulesManagerComponent: React.FC<
  BusinessRulesManagerComponentProps
> = ({ componentName, onClose, isOpen }) => {
  const [ruleName, setRuleName] = useState<string>('defaultRule');
  const dispatch = useDispatch();
  const container = useContext(InversifyContainerProviderContext);
  const businessRules: IBusinessRules = useSelector(
    getBusinessRules(componentName),
  );
  ...
  const onSave = async () => {
    await dispatch.pages.updateBusinessRule({
      componentName,
      ruleName,
      message,
      severity: 'error',
